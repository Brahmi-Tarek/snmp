                             ...
                             size_t send_len, const Address & address)
{
  ...
  else
  {
	...
	LOG("Snmp transport: getaddrinfo returns (errno) (str)");
	LOG(errno);
	LOG(strerror(errno));
	LOG_END;
	return -1;
    ...
    memcpy(&agent_addr, res->ai_addr, res->ai_addrlen);
			  ...
			  OctetStr &engine_id, bool process_msg = true)
{
  ...
  else if (from_addr.ss_family == AF_INET6)
  {
    ...
    getnameinfo((struct sockaddr *)&from_addr, fromlen,
		tmp_buffer, sizeof(tmp_buffer)-1, NULL, 0, NI_NUMERICHOST);
